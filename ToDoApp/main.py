...
async def get_all(db:db_dependency):
    ...
    if todo:
        return todo
    raise HTTPException({"error":"no todos"}, status_code = status.HTTP_404_NOTFOUND)
...
async def get_todo_by_id(id:int, db:db_dependency):
    ...
    if todo :
        return todo
    raise HTTPException({"error":"there is no todo with ids"} , status_code = status.HTTP_404_NOT_FOUND)
...
async def create_todo(data:TodosBodyRequest , db : db_dependency):
    ...
    if new_todo.id:
        return new_todo
    raise HTTPException({"details":"there is error in creation "}, status_code = status.HTTP_400_BAD_REQUEST)
...
async def create_todo(id:int , db : db_dependency):
